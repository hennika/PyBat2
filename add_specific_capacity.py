...
def incremental(df, char_mass):
    ...
    charge_incr_float = string_to_float.str_to_float(df['charge_incr'].tolist())  # Extracting incremental discharge as float
    charge_incr_spec = np.divide(charge_incr_float,float(char_mass) / 1000)  # Divide by mass in grams to obtain specific capacity.

    cap_incr_float = string_to_float.str_to_float(df['cap_incr'].tolist())  # Extracting incremental discharge as float
...
def cyclebased(df, char_mass):
    cycle_incr_float = string_to_float.str_to_float(df['cycle'].tolist())  # Extracting incremental cycle number as float
    discharge_incr_float = string_to_float.str_to_float(
    ...
    discharge_spec, charge_spec, cycles = fix_uneven_length.remove_last(discharge_spec, charge_spec, cycles,
    ...
    discharge_spec, charge_spec, cycles = fix_uneven_length.fill_none(discharge_spec, charge_spec, cycles, target=len(
...
def add_diffcap (df):
    ...
    volt_corr, diff_cap = fix_uneven_length.fill_none(volt_corr, diff_cap, target=len(df['potential'])) # This will add to "None" values to diff_cap variable, as it is two values shorter
